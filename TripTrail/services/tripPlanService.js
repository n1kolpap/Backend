 ...
 * @param {Object} tripPlanData - The data for the trip plan.
 * @returns {Promise<Object>} - The created trip plan.
 */
export const createTripPlan = async (userId, tripPlanData) => {
    try {
        const tripPlan = new TripPlan({ ...tripPlanData, user: userId });
        await tripPlan.save();
        return tripPlan;
    } catch (error) {
...
export const getTripPlanById = async (userId, tripId) => {
    try {
        const tripPlan = await TripPlan.findOne({ _id: tripId, user: userId });
        if (!tripPlan) {
...
export const deleteTripPlan = async (userId, tripId) => {
    try {
        const result = await TripPlan.deleteOne({ _id: tripId, user: userId });
